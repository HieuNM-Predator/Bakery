    ...
    <div class="d-flex bd-highlight mb-3">
        ...
        <nav class="navbar navbar-light bg-light">
            ...
            <form class="form-inline" method="get">
                ...
                <input id="myInput" name="myInput" class="form-control mr-sm-2" type="search" placeholder="Nhập tên tài khoản" aria-label="Tìm kiếm" value="<%=itemSearch%>">
            ...
            <tbody>
                <%
                     if(isnull(itemSearch) or trim(itemSearch) = "") Then
                        ...
                        cmdPrep.CommandText = "SELECT Id, TenTK, MatKhau, VaiTro FROM TAIKHOAN ORDER BY Id OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
    ...
    <nav aria-label="Page Navigation">
        <ul class="pagination pagination-sm justify-content-center my-5">
            ...
            'kiem tra trang hien tai co >=2
                    if(Clng(page)>=2) then
            %>
                    <li class="page-item"><a class="page-link" href="AccountManagement.asp?page=<%=Clng(page)-1%>">Previous</a></li>
            <%    
                    end if 
                    for i= 1 to pages
            %>
                        <li class="page-item <%=checkPage(Clng(i)=Clng(page),"active")%>"><a class="page-link" href="AccountManagement.asp?page=<%=i%>"><%=i%></a></li>
            <%
                    next
                    if (Clng(page)<pages) then

            %>
                    <li class="page-item"><a class="page-link" href="AccountManagement.asp?page=<%=Clng(page)+1%>">Next</a></li>
            <%
                    end if    
                end if
            %>
